 ...
 * @suppress
 */
public object FfiConverterTypeImage: FfiConverterRustBuffer<Image> {
    override fun read(buf: ByteBuffer): Image {
        return Image(
            FfiConverterString.read(buf),
            FfiConverterString.read(buf),
            FfiConverterString.read(buf),
 ...
 * @suppress
 */
public object FfiConverterSequenceTypeImage: FfiConverterRustBuffer<List<Image>> {
    override fun read(buf: ByteBuffer): List<Image> {
        val len = buf.getInt()
        return List<Image>(len) {
            FfiConverterTypeImage.read(buf)
        }
    }

    override fun allocationSize(value: List<Image>): ULong {
        val sizeForLength = 4UL
        val sizeForItems = value.map { FfiConverterTypeImage.allocationSize(it) }.sum()
        return sizeForLength + sizeForItems
    }

    override fun write(value: List<Image>, buf: ByteBuffer) {
        buf.putInt(value.size)
        value.iterator().forEach {
            FfiConverterTypeImage.write(it, buf)
        }
    }
}
