...
**生成时间**: 2026-01-15
---
### 2026-02-03 更新 (元数据可靠性与扫描性能优化)
**修复与优化**:
- **智能元数据缓存**: 引入 `mtime` 和 `size` 校验机制。在日常启动时通过数据库缓存直接读取图片尺寸，避免对 68k+ 文件的重复 IO 扫描。
- **强制重扫描模式 (Force Rescan)**: 在“欢迎界面”首次设置库或手动刷新时，开启同步尺寸读取（Synchronous IO），确保首次索引的完整性。
- **UI 零崩溃保障**: 修复了 `MetadataPanel` 中由于元数据缺失（undefined）导致的 `toUpperCase()` 调用崩溃问题。
- **异步补全机制**: 增加 `metadata-updated` 事件。后台任务完成尺寸提取后，前端 UI 会实时局部更新，无需用户手动刷新。
- **单文件主动修复**: 详情面板增加“自动补全”逻辑。当用户选中一个缺失尺寸的文件时，前端将主动触发 `scan_file` 进行单点修复。
