...
export const AddToTopicModal: React.FC<AddToTopicModalProps> = ({ topics, onConfirm, onClose, t }) => {
    ...
    return (
        <div className="bg-content rounded-xl p-6 shadow-2xl border border-subtle w-96 max-h-[500px] flex flex-col animate-zoom-in">
            <h3 className="font-bold text-lg mb-4 text-gray-900 dark:text-white">{t('sidebar.topics')}</h3>
            <div className="flex-1 overflow-y-auto bg-panel rounded-xl p-2 mb-4 max-h-[300px]">
                ...
                {mainTopics.map((topic: any) => {
                    ...
                    return (
                        <div key={topic.id} className="mb-1">
                            <div
                                className={`flex items-center p-2 rounded-lg cursor-pointer transition-colors ${isSelected ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400' : 'hover:bg-surface text-gray-800 dark:text-gray-200'}`}
                                onClick={() => setSelectedId(topic.id)}
                            >
                                <div
                                    className={`p-1 mr-1 rounded hover:bg-surface ${hasSubs ? 'visible' : 'invisible'}`}
                            ...
                            {hasSubs && isExpanded && (
                                <div className="ml-6 border-l border-subtle pl-2 mt-1 space-y-1">
                                    {subTopics.map((sub: any) => (
                                        <div
                                            key={sub.id}
                                            className={`flex items-center p-2 rounded-lg cursor-pointer transition-colors ${selectedId === sub.id ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400' : 'hover:bg-surface text-gray-800 dark:text-gray-200'}`}
            ...
            <div className="flex justify-end space-x-2">
                ...
                <button
                    ...
                    disabled={!selectedId}
                    className={`px-3 py-1.5 rounded-lg text-sm text-white transition-colors ${selectedId ? 'bg-blue-600 hover:bg-blue-700' : 'bg-gray-400 cursor-not-allowed'}`}
                >
                    {t('settings.confirm')}
                </button>
            </div>
