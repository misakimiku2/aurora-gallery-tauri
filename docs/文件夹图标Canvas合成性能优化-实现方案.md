...
| 层 | 内容 | 合成时机 | 缓存 |
|---|---|---|---|
| `back` | 后板（含 light 阴影） | 同步 | `layerCache`：`(back, category, theme, size, res)` |
| `front` | 前板(3D 投影) + 图标 | 同步 | `layerCache`：`(front, category, theme, size, res)` |
| `shell` | back + front（无预览无角标） | 同步 | `layerCache`：`(shell|…)` |
| `staticBody` | back + 三张灰阶占位卡 + front（无角标） | 同步 | `layerCache`：`(static, category, theme, size, res)` |
| `full` | back + 三张真实预览卡 + front + 角标 | 异步（idle 队列） | `fullCache` LRU：`(folderId, theme, size, res, count, previewSrcs)` |
...
- **修复**（`renderRes()`，spriteCache）+ `applySmoothing()`（composer）：
  ...
  - 可见 canvas backing 也用同一 `res`（`setTransform(res,0,0,res,pad*res,pad*res)`，pad 为 CANVAS_SCALE 留白）；
...
| FULL_LIMIT | 96（v2 计划 256≈100MB 过大，2x 后降为 96 ≈ 40MB） |
